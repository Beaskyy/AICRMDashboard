...
const getClient = () => {
  ...
  if (!apiKey) {
    throw new ApiError(
      503,
      "Gemini API Key not configured. Add GEMINI_API_KEY to the backend .env file",
    );
  }
...
export const generateEmail = async ({ lead, purpose, tone, sender }) => {
  ...
  Write a professional sales mail.
...
export const generateSalesIngsights = async (pipelineStats) => {
